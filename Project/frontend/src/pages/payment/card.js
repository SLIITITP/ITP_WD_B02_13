		...
		if (cardno.length === 16 && cardcvc.length === 3 && cardno.match(/^[0-9]+$/) && cardcvc.match(/^[0-9]+$/)) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					axios
						.post("http://localhost:8070/cardDetails/add")
						.then((res) => {
							console.log(res);
		...
		<div>
			...
			<div className="cardaddhead">
				...
				<div className="frame">
					<div className="mb-6">
						<div>
							<label for="Card Type" class="block mb-2 text-sm font-medium text-white ">
								Card Type
							</label>
						...
						<div>
							<label for="Card Number" class="block mb-2 text-sm font-medium text-white ">
								Card Number
							</label>
							<input
								type="text"
						...
						<div>
							<label for="Cvc" class="block mb-2 text-sm font-medium text-white ">
								CVC
							</label>
							<input
								type="text"
					...
					<label for="quantity" style={{ color: "red", fontSize: "11px" }}>
						Note: We wil not save your card deatils and this section is only for verification purposes only, Your
